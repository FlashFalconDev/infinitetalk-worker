...
class InfiniteTalkWorker:
    def __init__(self):
        ...
        # 初始化
        logger.info("=" * 70)
        logger.info("🚀 初始化 InfiniteTalk Worker v7.3.3")
    ...
    def _get_api_endpoints(self):
        """✅ 動態獲取 API 端點（心跳用 /ai/api，其他用 /aigen/api）"""
        return {
    ...
    def _switch_to_backup(self):
        ...
        if self.current_base == self.primary_base:
            logger.warning("⚠️  主 API 連線失敗，切換到備用 API")
    ...
    def _make_request(self, method, endpoint_key, **kwargs):
        ...
        try:
            ...
            if self.primary_fail_count > 0:
                self.primary_fail_count = 0
                logger.debug(f"✅ API 連線正常，重置失敗計數")
            ...
            logger.warning(f"⚠️  連線失敗: {type(e).__name__}")
            
            # 只有主 API 失敗才計數
            if self.current_base == self.primary_base:
                self.primary_fail_count += 1
                logger.warning(f"⚠️  主 API 連線失敗次數: {self.primary_fail_count}/{self.max_fails_before_switch}")
                ...
                if self.primary_fail_count >= self.max_fails_before_switch:
                    ...
                    try:
                        ...
                        logger.error(f"❌ 備用 API 也連線失敗: {e2}")
                        return None
    ...
    def _send_heartbeat(self):
        ...
        try:
            ...
            if response and response.status_code == 200:
                ...
                logger.warning(f"⚠️  心跳響應異常: {response.status_code if response else 'None'}")
                return False
    ...
    def fetch_task(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️  獲取任務響應異常: HTTP {response.status_code}")
                return []
